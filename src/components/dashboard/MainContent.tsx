...
export const MainContent: React.FC<MainContentProps> = ({ setSidebarOpen }) => {
  ...
  const displayTemp = weather?.temperature ?? 72; // Fallback to reasonable default
  const displayCondition = weather?.condition ?? (isLoadingWeather ? 'Loading...' : 'Clear');
  const unit = '°F'; // Default, can integrate units later
